...
def main(
    ...
    processed_files = 0
    ignore_list_path = aist_root_dir / "ignore_mdm_list.txt"
    ...
    for folder_name in folder_names:
        ...
        files = [
            f
            for f in Path(data_dir).glob("**/*.[np][pk][lzy]")
        ...
        for filename in tqdm(filter_files):
            try:
                ...
                if filename.suffix == ".npy":
                    ...
                    motion_data = np.load(filename, allow_pickle=True).item()

                    pose_euler = np.array(motion_data['thetas']).reshape(-1, 24, 3)
                    B = pose_euler.shape[0]
                    # print(f"aist_pose shape: {pose_euler.shape}")
                    aist_trans = np.array(motion_data['root_translation']).squeeze()
                    # print(f"aist_trans shape: {aist_trans.shape}")
                    ...
                    pose_aa[:, :3] = new_root
                    trans = aist_trans.dot(transform.as_matrix().T)
                    # import pdb;pdb.set_trace()

                    trans[:, 2] -= (trans[0, 2] - 0.92)
                    # Translated comment.
                    if trans[:, 2].max() > 1.4 or trans[:, 2].min() < 0.5:
                        print(f"Skipping file {outpath} due to trans[:, 2] out of range.")
                        continue

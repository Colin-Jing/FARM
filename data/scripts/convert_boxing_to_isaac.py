...
def main(
    ...
    folder_names = [
        "1_jab", "2_cross", "3_hook", "4_right_hook", "5_left_uppercut", "6_right_uppercut", 
        "7_duck_right", "8_duck_left", "9_slip"
    ] 
        ...
        for filename in tqdm(filter_files):
            try:
                ...
                if filename.suffix == ".pt":
                    ...
                    body_pose_global = smpl_global['body_pose'].cpu().numpy()   # [B, 63]
                    global_orient = smpl_global['global_orient'].cpu().numpy()  # [B, 3]
                    transl_global = smpl_global['transl'].cpu().numpy()  # [B, 3]
                    body_pose_global = body_pose_global # [B, 69] (root_orient + pose_body)

                    pose_aa = np.concatenate([global_orient, body_pose_global], axis=1)  # [B, 66] (root_orient + pose_body)
                    trans = transl_global  # [B, 3] 

                    # B = pose_aa_array.shape[0]
                    # pose_aa = sRot.from_euler('XYZ', pose_aa_array.reshape(-1, 3), degrees=True).as_rotvec().reshape(B, 66)
                ...
                pose_aa = torch.tensor(pose_aa)
                aist_trans = torch.tensor(trans)
                ...
                print("root_trans_offset.shape = ", root_trans_offset.numpy().shape)
                motion_dict = {
                    "pose_aa": pose_aa,
                    "trans": root_trans_offset.numpy()
                }
                # Translated comment.
                pkl_path = str(outpath).replace(".npy", ".pkl")
                joblib.dump(motion_dict, pkl_path)

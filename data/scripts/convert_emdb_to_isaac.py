...
def main(
    ...
    processed_files = 0
    ignore_list_path = aist_root_dir / "ignore_list_emdb.txt"
    ...
    for filename in tqdm(filter_files):
        try:
            ...
            if filename.suffix == ".pkl":
                ...
                # aist_pose = motion_data['smpl_poses']
                # scale = motion_data['smpl_scaling']
                # aist_trans = motion_data['smpl_trans'] / scale
                ...
                entry_data = joblib.load(filename)['smpl']


                poses_root = entry_data['poses_root']
                pose_aa = entry_data['poses_body'][:, :63]  # [B, 63] (pose_body)
                pose_aa = np.concatenate([poses_root, pose_aa], axis=1)  # [B, 66] (root_orient + pose_body)
                ...
                trans[:, 2] = trans[:, 2] - (trans[0, 2] - 0.92) # 0.92
                betas = np.zeros(10)
                mocap_fr = 30
